...
public class darius
{
	...
	*****************************************************/
	...
	public static Memory_WriteAddress darius_sound2_writemem[]={
		new Memory_WriteAddress(MEMPORT_MARKER, MEMPORT_DIRECTION_WRITE | MEMPORT_TYPE_MEM | MEMPORT_WIDTH_8),	new Memory_WriteAddress( 0x0000, 0xffff, MWA_NOP ),	/* writes rom whenever interrupt occurs - as no stack */
		/* yes, no RAM */
		new Memory_WriteAddress(MEMPORT_MARKER, 0)
	};
	
	public static vclk_interruptPtr darius_adpcm_int = new vclk_interruptPtr() { public void handler(int data) 
	{
		if (nmi_enable != 0)
		{
	...
	(
		1,				/* 1 chip */
		384000, 			/* 384KHz */
		new vclk_interruptPtr[] { darius_adpcm_int},	/* interrupt function */
	...
	public static IO_ReadPort darius_sound2_readport[]={
		new IO_ReadPort(MEMPORT_MARKER, MEMPORT_DIRECTION_READ | MEMPORT_TYPE_IO | MEMPORT_WIDTH_8),	new IO_ReadPort( 0x00, 0x00, adpcm_command_read ),
	...
	public static IO_WritePort darius_sound2_writeport[]={
		new IO_WritePort(MEMPORT_MARKER, MEMPORT_DIRECTION_WRITE | MEMPORT_TYPE_IO | MEMPORT_WIDTH_8),	new IO_WritePort( 0x00, 0x00, adpcm_nmi_disable ),
	...
	***********************************************************/
	...
	PORT_START(); 	/* IN0 */
		...
		PORT_START(); 	/* IN2 */
		PORT_BIT( 0x01, IP_ACTIVE_HIGH, IPT_COIN1 );	PORT_BIT( 0x02, IP_ACTIVE_HIGH, IPT_COIN2 );	PORT_BIT( 0x04, IP_ACTIVE_LOW,  IPT_START1 );	PORT_BIT( 0x08, IP_ACTIVE_LOW,  IPT_START2 );	PORT_BIT( 0x10, IP_ACTIVE_LOW,  IPT_SERVICE1 );	PORT_BIT( 0x20, IP_ACTIVE_LOW,  IPT_TILT );	PORT_BIT( 0x40, IP_ACTIVE_LOW,  IPT_UNKNOWN );	PORT_BIT( 0x80, IP_ACTIVE_LOW,  IPT_UNKNOWN );
		PORT_START(); 	/* DSW */
		...
		PORT_DIPSETTING(      0x0200, "Easy" );	PORT_DIPSETTING(      0x0300, "Medium" );	PORT_DIPSETTING(      0x0100, "Hard" );	PORT_DIPSETTING(      0x0000, "Hardest" );	PORT_DIPNAME( 0x0c00, 0x0c00, DEF_STR( "Bonus_Life") );
		PORT_DIPSETTING(      0x0000, "?K, ?K" );	PORT_DIPSETTING(      0x0400, "?K, ?K" );	PORT_DIPSETTING(      0x0800, "?K, ?K" );	PORT_DIPSETTING(      0x0c00, "?K, ?K" );	PORT_DIPNAME( 0x3000, 0x3000, DEF_STR( "Lives") );
		PORT_DIPSETTING(      0x3000, "3" );	PORT_DIPSETTING(      0x2000, "4" );	PORT_DIPSETTING(      0x1000, "5" );	PORT_DIPSETTING(      0x0000, "6" );	PORT_DIPNAME( 0x4000, 0x4000, DEF_STR( "Unknown") );
		PORT_DIPSETTING(      0x4000, DEF_STR( "Off") );
		PORT_DIPSETTING(      0x0000, DEF_STR( "On") );
		PORT_DIPNAME( 0x8000, 0x8000, "Continue" );	PORT_DIPSETTING(      0x0000, DEF_STR( "No") );
	...
	**************************************************************/
	...
	(
		...
		new ReadHandlerPtr[] { 0, 0 },		/* portA read */
		new ReadHandlerPtr[] { 0, 0 },
		new WriteHandlerPtr[] { darius_write_portA0, darius_write_portA1 },	/* portA write */
		new WriteHandlerPtr[] { darius_write_portB0, darius_write_portB1 },	/* portB write */
		new WriteYmHandlerPtr[] { irqhandler, 0 }
	);
	...
	***************************************************************************/
	...
	static RomLoadPtr rom_darius = new RomLoadPtr(){ public void handler(){ 
		...
		ROM_REGION( 0x60000, REGION_GFX1, ROMREGION_DISPOSE );	/* There are THREE of each SCR gfx rom on the actual board,
		...
		ROM_REGION( 0xc0000, REGION_GFX2, ROMREGION_DISPOSE );	ROM_LOAD32_BYTE( "a96_44.179",   0x00000, 0x10000, 0xbbc18878 )	/* 16x16 sprites */
		...
		ROM_LOAD16_BYTE( "a96_55.144",   0x0001, 0x4000, 0x771e4d98 )
	 	...
	 	ROM_REGION( 0x1000, REGION_USER1, 0 );	ROM_LOAD16_BYTE( "a96-24.163",   0x0000, 0x0400, 0x0fa8be7f )	/* proms, currently unused */
	...
	static RomLoadPtr rom_dariusj = new RomLoadPtr(){ public void handler(){ 
		...
		ROM_REGION( 0x60000, REGION_GFX1, ROMREGION_DISPOSE );	ROM_LOAD16_BYTE( "a96_48.24",    0x00000, 0x10000, 0x39c9b3aa )	/* 8x8 SCR tiles */
		ROM_LOAD16_BYTE( "a96_49.25",    0x20000, 0x10000, 0x37a7d88a )
		ROM_LOAD16_BYTE( "a96_50.26",    0x40000, 0x10000, 0x75d738e4 )
		ROM_LOAD16_BYTE( "a96_51.47",    0x00001, 0x10000, 0x1bf8f0d3 )
		ROM_LOAD16_BYTE( "a96_52.48",    0x20001, 0x10000, 0x2d9b2128 )
