...
public class srmp2
{
	...
	***************************************************************************/
	...
	static public static nvramPtr srmp2_nvram_handler  = new nvramPtr() { public void handler(Object file, int read_or_write) 
	{
		if (read_or_write != 0)
		{
	...
	public static ReadHandlerPtr srmp2_dsw1_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
		return readinputport(2);
	} };
	
	
	public static ReadHandlerPtr srmp2_dsw2_r  = new ReadHandlerPtr() { public int handler(int offset)
	{
	...
	*/
	...
	public static vclk_interruptPtr srmp2_adpcm_int = new vclk_interruptPtr() { public void handler(int num) 
	{
		UBytePtr ROM = memory_region(REGION_SOUND1);
	...
	***************************************************************************/
	...
	PORT_START(); 			/* Coinnage (0) */
		...
		PORT_DIPSETTING(    0x01, DEF_STR( "Off") );
		PORT_DIPSETTING(    0x00, DEF_STR( "On") );
		PORT_SERVICE( 0x02, IP_ACTIVE_LOW );	PORT_DIPNAME( 0x0c, 0x0c, DEF_STR( "Coinage") );
		...
		PORT_START(); 			/* INPUT1 (3) */
		PORT_BIT(  0x01, IP_ACTIVE_HIGH, IPT_UNKNOWN );	PORT_BITX( 0x02, IP_ACTIVE_HIGH, 0, "P1 Small",       KEYCODE_BACKSPACE, IP_JOY_NONE );	PORT_BITX( 0x04, IP_ACTIVE_HIGH, 0, "P1 Double Up",   KEYCODE_RSHIFT,    IP_JOY_NONE );	PORT_BITX( 0x08, IP_ACTIVE_HIGH, 0, "P1 Big",         KEYCODE_ENTER,     IP_JOY_NONE );	PORT_BITX( 0x10, IP_ACTIVE_HIGH, 0, "P1 Take Score",  KEYCODE_RCONTROL,  IP_JOY_NONE );	PORT_BITX( 0x20, IP_ACTIVE_HIGH, 0, "P1 Flip",        KEYCODE_X,         IP_JOY_NONE );	PORT_BITX( 0x40, IP_ACTIVE_HIGH, 0, "P1 Last Chance", KEYCODE_RALT,      IP_JOY_NONE );	PORT_BIT(  0x80, IP_ACTIVE_HIGH, IPT_UNKNOWN );
		PORT_START(); 			/* INPUT1 (4) */
		PORT_BITX( 0x01, IP_ACTIVE_HIGH, 0, "P1 K",   KEYCODE_K,     IP_JOY_NONE );	PORT_BITX( 0x02, IP_ACTIVE_HIGH, 0, "P1 Ron", KEYCODE_Z,     IP_JOY_NONE );	PORT_BITX( 0x04, IP_ACTIVE_HIGH, 0, "P1 G",   KEYCODE_G,     IP_JOY_NONE );	PORT_BITX( 0x08, IP_ACTIVE_HIGH, 0, "P1 Chi", KEYCODE_SPACE, IP_JOY_NONE );	PORT_BITX( 0x10, IP_ACTIVE_HIGH, 0, "P1 C",   KEYCODE_C,     IP_JOY_NONE );	PORT_BIT(  0x20, IP_ACTIVE_HIGH, IPT_UNKNOWN );	PORT_BITX( 0x40, IP_ACTIVE_HIGH, 0, "P1 L",   KEYCODE_L,     IP_JOY_NONE );	PORT_BIT(  0x80, IP_ACTIVE_HIGH, IPT_UNKNOWN );
		PORT_START(); 			/* INPUT1 (5) */
		PORT_BITX( 0x01, IP_ACTIVE_HIGH, 0, "P1 H",     KEYCODE_H,        IP_JOY_NONE );	PORT_BITX( 0x02, IP_ACTIVE_HIGH, 0, "P1 Pon",   KEYCODE_LALT,     IP_JOY_NONE );	PORT_BITX( 0x04, IP_ACTIVE_HIGH, 0, "P1 D",     KEYCODE_D,        IP_JOY_NONE );	PORT_BITX( 0x08, IP_ACTIVE_HIGH, 0, "P1 Start", KEYCODE_1,        IP_JOY_NONE );	PORT_BITX( 0x10, IP_ACTIVE_HIGH, 0, "P1 I",     KEYCODE_I,        IP_JOY_NONE );	PORT_BITX( 0x20, IP_ACTIVE_HIGH, 0, "P1 Kan",   KEYCODE_LCONTROL, IP_JOY_NONE );	PORT_BITX( 0x40, IP_ACTIVE_HIGH, 0, "P1 E",     KEYCODE_E,        IP_JOY_NONE );	PORT_BITX( 0x80, IP_ACTIVE_HIGH, 0, "P1 M",     KEYCODE_M,        IP_JOY_NONE );
		PORT_START(); 			/* INPUT1 (6) */
		PORT_BITX( 0x01, IP_ACTIVE_HIGH, 0, "P1 A",     KEYCODE_A,      IP_JOY_NONE );	PORT_BITX( 0x02, IP_ACTIVE_HIGH, 0, "P1 Bet",   KEYCODE_2,      IP_JOY_NONE );	PORT_BITX( 0x04, IP_ACTIVE_HIGH, 0, "P1 J",     KEYCODE_J,      IP_JOY_NONE );	PORT_BITX( 0x08, IP_ACTIVE_HIGH, 0, "P1 Reach", KEYCODE_LSHIFT, IP_JOY_NONE );	PORT_BITX( 0x10, IP_ACTIVE_HIGH, 0, "P1 F",     KEYCODE_F,      IP_JOY_NONE );	PORT_BITX( 0x20, IP_ACTIVE_HIGH, 0, "P1 N",     KEYCODE_N,      IP_JOY_NONE );	PORT_BITX( 0x40, IP_ACTIVE_HIGH, 0, "P1 B",     KEYCODE_B,      IP_JOY_NONE );	PORT_BIT(  0x80, IP_ACTIVE_HIGH, IPT_UNKNOWN );
		PORT_START(); 			/* INPUT1 (7) */
		PORT_BIT( 0x01, IP_ACTIVE_LOW, IPT_UNKNOWN );	PORT_BIT( 0x02, IP_ACTIVE_LOW, IPT_SERVICE2 );	PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_UNKNOWN );	PORT_BIT( 0x08, IP_ACTIVE_LOW, IPT_SERVICE3 );	PORT_BIT( 0x10, IP_ACTIVE_LOW, IPT_UNKNOWN );	PORT_BIT( 0x20, IP_ACTIVE_LOW, IPT_UNKNOWN );	PORT_BIT( 0x40, IP_ACTIVE_LOW, IPT_UNKNOWN );	PORT_BIT( 0x80, IP_ACTIVE_LOW, IPT_UNKNOWN );INPUT_PORTS_END(); }}; 
	...
	***************************************************************************/
	...
	(
		...
		20000000/8,					/* 2.50 MHz ??? */
		new int[] { 40 },
		new ReadHandlerPtr[] { srmp2_dsw1_r },			/* Input A: DSW 1 */
		new ReadHandlerPtr[] { srmp2_dsw2_r },			/* Input B: DSW 2 */
	...
	{
		new GfxDecodeInfo( REGION_GFX1, 0, charlayout, 0, 16*32 ),
		new GfxDecodeInfo( -1 )
	};
	...
	(
		new MachineCPU[] {
			new MachineCPU(
				CPU_M68000,
				8000000,
				srmp2_readmem, srmp2_writemem, null, null,
	...
	***************************************************************************/
	...
	static RomLoadPtr rom_srmp2 = new RomLoadPtr(){ public void handler(){ 
		...
		ROM_LOAD       ( "ubo-4.60",  0x000000, 0x040000, 0xcb6f7cce );	ROM_LOAD       ( "ubo-5.61",  0x040000, 0x040000, 0x7b48c540 );	ROM_LOAD16_BYTE( "uco-8.64",  0x080000, 0x040000, 0x1ca1c7c9 )
		ROM_LOAD16_BYTE( "uco-9.65",  0x080001, 0x040000, 0xef75471b )
		ROM_LOAD       ( "ubo-6.62",  0x100000, 0x040000, 0x6c891ac5 );	ROM_LOAD       ( "ubo-7.63",  0x140000, 0x040000, 0x60a45755 );	ROM_LOAD16_BYTE( "uco-10.66", 0x180000, 0x040000, 0xcb6bd857 )
		...
		ROM_LOAD( "uc-1o.12", 0x000000, 0x000400, 0xfa59b5cb );	ROM_LOAD( "uc-2o.13", 0x000400, 0x000400, 0x50a33b96 );ROM_END(); }}; 
	
	
	/*   ( YEAR   NAME  PARENT  MACHINE    INPUT     INIT     MONITOR  COMPANY  FULLNAME ) */
	public static GameDriver driver_srmp2	   = new GameDriver("1987"	,"srmp2"	,"srmp2.java"	,rom_srmp2,null	,machine_driver_srmp2	,input_ports_srmp2	,init_srmp2	,ROT0_16BIT	,	"Seta", "Super Real Mahjong P2 (Japan)" )
}
