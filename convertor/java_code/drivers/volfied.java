...
public class volfied
{
	...
	***********************************************************/
	...
	PORT_START(); 	/* DSW A */
		...
		PORT_DIPSETTING(    0x0c, "20k,40k,120k,480k,2400k" );	PORT_DIPSETTING(    0x04, "50k,150k,600k,3000k" );	PORT_DIPSETTING(    0x08, "70k,280k,1400k" );	PORT_DIPSETTING(    0x00, "100k,500k" );	PORT_DIPNAME( 0x30, 0x30, DEF_STR( "Lives") );
		...
		PORT_DIPSETTING(    0x00, DEF_STR( "On") );
		PORT_DIPNAME( 0x80, 0x80, "Language" );	PORT_DIPSETTING(    0x80, "Japanese" );	PORT_DIPSETTING(    0x00, "English" );
		PORT_START(); 	/* f00007 */
		...
		PORT_START(); 	/* f0000b */
		PORT_BIT( 0x01, IP_ACTIVE_LOW, IPT_TILT );	PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_JOYSTICK_UP    | IPF_4WAY );	PORT_BIT( 0x08, IP_ACTIVE_LOW, IPT_JOYSTICK_DOWN  | IPF_4WAY );	PORT_BIT( 0x10, IP_ACTIVE_LOW, IPT_JOYSTICK_LEFT  | IPF_4WAY );	PORT_BIT( 0x20, IP_ACTIVE_LOW, IPT_JOYSTICK_RIGHT | IPF_4WAY );	PORT_BIT( 0x40, IP_ACTIVE_LOW, IPT_BUTTON1 );
		PORT_START(); 	/* f0000d */
		PORT_BIT( 0x02, IP_ACTIVE_LOW, IPT_JOYSTICK_UP    | IPF_4WAY | IPF_PLAYER2 );	PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_JOYSTICK_DOWN  | IPF_4WAY | IPF_PLAYER2 );	PORT_BIT( 0x08, IP_ACTIVE_LOW, IPT_JOYSTICK_LEFT  | IPF_4WAY | IPF_PLAYER2 );	PORT_BIT( 0x10, IP_ACTIVE_LOW, IPT_JOYSTICK_RIGHT | IPF_4WAY | IPF_PLAYER2 );	PORT_BIT( 0x20, IP_ACTIVE_LOW, IPT_BUTTON1 | IPF_PLAYER2 );INPUT_PORTS_END(); }}; 
	...
	**************************************************************/
	...
	(
		1,         /* 1 chip */
		3000000,   /* 3 MHz? */
		new int[] { YM2203_VOL(60,15) },
		new ReadHandlerPtr[] { input_port_0_r },    /* DSW A  */
		new ReadHandlerPtr[] { input_port_1_r },    /* DSW B  */
	...
	***********************************************************/
	...
	static MachineDriver machine_driver_volfied = new MachineDriver
	(
		...
		new MachineCPU[] {
			new MachineCPU(
				CPU_M68000,
				8000000,   /* 8MHz? */
				volfied_readmem, volfied_writemem, null, null,
				m68_level4_irq, 1
			),
			new MachineCPU(
				CPU_Z80,   /* sound CPU, required to run the game  */
				4000000,   /* 4MHz? */
				z80_readmem, z80_writemem, 0, null,
				ignore_interrupt, 0
			)
		},
	...
	***************************************************************************/
	...
	static RomLoadPtr rom_volfied = new RomLoadPtr(){ public void handler(){ 
		...
		ROM_RELOAD     (                 0xa0000, 0x10000 );	ROM_LOAD16_BYTE( "c04-09.bin",   0x80001, 0x10000, 0xc78cf057 )
		ROM_RELOAD     (                 0xa0001, 0x10000 );
		ROM_REGION( 0x10000, REGION_CPU2, 0 );    /* sound cpu */
		ROM_LOAD( "c04-06.bin", 0x0000, 0x8000, 0xb70106b2 );ROM_END(); }}; 
