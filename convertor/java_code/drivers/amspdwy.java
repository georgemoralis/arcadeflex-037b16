...
public class amspdwy
{
	...
	*/
	...
	public static IO_ReadPort amspdwy_readport[]={
		new IO_ReadPort(MEMPORT_MARKER, MEMPORT_DIRECTION_READ | MEMPORT_TYPE_IO | MEMPORT_WIDTH_8),	new IO_ReadPort( 0x0000, 0x7fff, amspdwy_port_r	),
	...
	***************************************************************************/
	...
	static InputPortPtr input_ports_amspdwy = new InputPortPtr(){ public void handler() { 
		...
		PORT_DIPNAME( 0x01, 0x00, "Character Test" );	PORT_DIPSETTING(    0x00, DEF_STR( "Off") );
		PORT_DIPSETTING(    0x01, DEF_STR( "On") );
		PORT_DIPNAME( 0x02, 0x00, "Show Arrows" );	PORT_DIPSETTING(    0x00, DEF_STR( "Off") );
		PORT_DIPSETTING(    0x02, DEF_STR( "On") );
		PORT_DIPNAME( 0x04, 0x04, DEF_STR( "Demo_Sounds") );
		PORT_DIPSETTING(    0x00, DEF_STR( "Off") );
		PORT_DIPSETTING(    0x04, DEF_STR( "On") );
		PORT_SERVICE( 0x08, IP_ACTIVE_HIGH );	PORT_DIPNAME( 0x10, 0x00, "Steering Test" );	PORT_DIPSETTING(    0x00, DEF_STR( "Off") );
	...
	//	PORT_DIPSETTING(    0x02, DEF_STR( "1C_1C") );
		...
		PORT_DIPSETTING(    0x00, "Easy" );	PORT_DIPSETTING(    0x04, "Normal" );	PORT_DIPSETTING(    0x08, "Hard" );	PORT_DIPSETTING(    0x0c, "Hardest" );	PORT_DIPNAME( 0x10, 0x00, "Time" );	PORT_DIPSETTING(    0x10, "45 sec" );	PORT_DIPSETTING(    0x00, "60 sec" );	PORT_BIT(     0xe0, IP_ACTIVE_HIGH, IPT_UNKNOWN );
		...
		PORT_BIT( 0x1f, IP_ACTIVE_HIGH, IPT_SPECIAL );	PORT_BIT_IMPULSE( 0x80, IP_ACTIVE_HIGH, IPT_COIN2, 2 );
		PORT_START(); 	// IN4 - Player 1&2 Pedals + YM2151 Sound Status
		PORT_BIT( 0x0f, IP_ACTIVE_HIGH, IPT_SPECIAL );	PORT_BIT( 0x10, IP_ACTIVE_LOW, IPT_BUTTON1 | IPF_PLAYER2 );	PORT_BIT( 0x20, IP_ACTIVE_LOW, IPT_BUTTON1 | IPF_PLAYER1 );	PORT_BIT( 0xc0, IP_ACTIVE_HIGH, IPT_SPECIAL );
	...
	***************************************************************************/
	...
	static RomLoadPtr rom_amspdwy = new RomLoadPtr(){ public void handler(){ 
		...
		ROM_LOAD( "game5807.u33", 0x00000, 0x8000, 0x88233b59 );	ROM_LOAD( "trks6092.u34", 0x10000, 0x8000, 0x74a4e7b7 );
		ROM_REGION( 0x10000, REGION_CPU2, 0 );	/* Sound Z80 Code */
		ROM_LOAD( "audi9463.u2", 0x00000, 0x8000, 0x61b0467e );
		ROM_REGION( 0x4000, REGION_GFX1, ROMREGION_DISPOSE );/* Layer + Sprites */
		ROM_LOAD( "hilo9b3c.5a", 0x0000, 0x1000, 0xf50f864c );	ROM_LOAD( "hihie12a.4a", 0x1000, 0x1000, 0x3d7497f3 );	ROM_LOAD( "lolo1d51.1a", 0x2000, 0x1000, 0x58701c1c );	ROM_LOAD( "lohi4644.2a", 0x3000, 0x1000, 0xa1d802b1 );ROM_END(); }}; 
	
	static RomLoadPtr rom_amspdwya = new RomLoadPtr(){ public void handler(){ 
		ROM_REGION( 0x18000, REGION_CPU1, 0 );	/* Main Z80 Code */
		ROM_LOAD( "game.u33",     0x00000, 0x8000, 0xfacab102 );	ROM_LOAD( "trks6092.u34", 0x10000, 0x8000, 0x74a4e7b7 );
		ROM_REGION( 0x10000, REGION_CPU2, 0 );	/* Sound Z80 Code */
		ROM_LOAD( "audi9463.u2", 0x00000, 0x8000, 0x61b0467e );
		ROM_REGION( 0x4000, REGION_GFX1, ROMREGION_DISPOSE );/* Layer + Sprites */
		ROM_LOAD( "hilo9b3c.5a", 0x0000, 0x1000, 0xf50f864c );	ROM_LOAD( "hihie12a.4a", 0x1000, 0x1000, 0x3d7497f3 );	ROM_LOAD( "lolo1d51.1a", 0x2000, 0x1000, 0x58701c1c );	ROM_LOAD( "lohi4644.2a", 0x3000, 0x1000, 0xa1d802b1 );ROM_END(); }}; 
