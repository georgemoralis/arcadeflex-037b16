...
public class discrete
{
	...
	void CLIB_DECL discrete_log(const char *text, ...)
	{
		...
		if (firstout != 0) disclogfile=fopen("discrete.log", "wa"); else disclogfile=fopen("discrete.log", "a");
		firstout=0;
	
	    if (disclogfile != 0) vfprintf(disclogfile, text, arg);
	    fprintf(disclogfile,"\n");
	    fclose(disclogfile);
	...
	public static ShStartPtr discrete_sh_start = new ShStartPtr() { public int handler(MachineSound msound) 
	{
	...
	public static ShStopPtr discrete_sh_stop = new ShStopPtr() { public void handler() 
	{
		int loop=0;
		if(!init_ok) return;
	...
	public static ShUpdatePtr discrete_sh_update = new ShUpdatePtr() { public void handler() 
	{
		if(!init_ok) return;
