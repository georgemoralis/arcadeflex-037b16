...
public class tilemap
{
	...
	static void blendbitmask16( UINT16 *dest, const UINT16 *source, const UINT8 *bitmask, int count )
	{
		...
		{
			UINT32 data = *bitmask++;
			if ((data & 0x80) != 0) dest[0] = alpha_blend16(dest[0], source[0]);
			if ((data & 0x40) != 0) dest[1] = alpha_blend16(dest[1], source[1]);
			if ((data & 0x20) != 0) dest[2] = alpha_blend16(dest[2], source[2]);
			if ((data & 0x10) != 0) dest[3] = alpha_blend16(dest[3], source[3]);
			if ((data & 0x08) != 0) dest[4] = alpha_blend16(dest[4], source[4]);
			if ((data & 0x04) != 0) dest[5] = alpha_blend16(dest[5], source[5]);
			if ((data & 0x02) != 0) dest[6] = alpha_blend16(dest[6], source[6]);
			if ((data & 0x01) != 0) dest[7] = alpha_blend16(dest[7], source[7]);
			if( --count == 0 ) break;
			source+=8;
			dest+=8;
		}
	...
	static void blendbitmask32( UINT32 *dest, const UINT32 *source, const UINT8 *bitmask, int count )
	{
		...
		{
			UINT32 data = *bitmask++;
			if ((data & 0x80) != 0) dest[0] = alpha_blend32(dest[0], source[0]);
			if ((data & 0x40) != 0) dest[1] = alpha_blend32(dest[1], source[1]);
			if ((data & 0x20) != 0) dest[2] = alpha_blend32(dest[2], source[2]);
			if ((data & 0x10) != 0) dest[3] = alpha_blend32(dest[3], source[3]);
			if ((data & 0x08) != 0) dest[4] = alpha_blend32(dest[4], source[4]);
			if ((data & 0x04) != 0) dest[5] = alpha_blend32(dest[5], source[5]);
			if ((data & 0x02) != 0) dest[6] = alpha_blend32(dest[6], source[6]);
			if ((data & 0x01) != 0) dest[7] = alpha_blend32(dest[7], source[7]);
			if( --count == 0 ) break;
			source+=8;
			dest+=8;
		}
					...
					if( tile_type!=prev_tile_type )
					{
						...
						if( prev_tile_type != TILE_TRANSPARENT )
						{
							if( prev_tile_type == TILE_MASKED )
							{
								...
								for(;;)
								{
									if (screen != 0) memcpybitmask( dest0, source0, mask0, count );
							...
							{ /* TILE_OPAQUE */
								...
								for(;;)
								{
									if (screen != 0) memcpy( dest0, source0, num_pixels*sizeof(DATA_TYPE) );
