...
public class segac2
{
	...
	******************************************************************************/
	...
	public static VhUpdatePtr segac2_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		int y, color;
	...
	static void drawline_tiles(UINT32 *tiles, UINT16 *bmap, int pri)
	{
		...
		for (column = 0; column < 41; column++, bmap += 8)
		{
			...
			if (((tile >> 15) & 1) == pri)
			{
				...
				if (!(tile & 0x0800))
				{
					col = (mytile >> 28) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(0)] = colbase + col;
					col = (mytile >> 24) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(1)] = colbase + col;
					col = (mytile >> 20) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(2)] = colbase + col;
					col = (mytile >> 16) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(3)] = colbase + col;
					col = (mytile >> 12) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(4)] = colbase + col;
					col = (mytile >> 8)  & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(5)] = colbase + col;
					col = (mytile >> 4)  & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(6)] = colbase + col;
					col = (mytile >> 0)  & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(7)] = colbase + col;
				}
	
				/* horizontal flip */
				else
				{
					col = (mytile >> 28) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(7)] = colbase + col;
					col = (mytile >> 24) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(6)] = colbase + col;
					col = (mytile >> 20) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(5)] = colbase + col;
					col = (mytile >> 16) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(4)] = colbase + col;
					col = (mytile >> 12) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(3)] = colbase + col;
					col = (mytile >> 8)  & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(2)] = colbase + col;
					col = (mytile >> 4)  & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(1)] = colbase + col;
					col = (mytile >> 0)  & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(0)] = colbase + col;
				}
			}
		}
	}
	...
	******************************************************************************/
	...
	INLINE void draw8pixs(UINT16 *bmap, int patno, int priority, int colbase, int patline)
	{
		...
		{
			col = (tile >> 28) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(0)] = colbase + col;
			col = (tile >> 24) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(1)] = colbase + col;
			col = (tile >> 20) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(2)] = colbase + col;
			col = (tile >> 16) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(3)] = colbase + col;
			col = (tile >> 12) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(4)] = colbase + col;
			col = (tile >> 8)  & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(5)] = colbase + col;
			col = (tile >> 4)  & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(6)] = colbase + col;
			col = (tile >> 0)  & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(7)] = colbase + col;
		}
	...
	INLINE void draw8pixs_hflip(UINT16 *bmap, int patno, int priority, int colbase, int patline)
	{
		...
		if ((colbase & 0x30) != 0x30 || !(segac2_vdp_regs[12] & 0x08))
		{
			col = (tile >> 28) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(7)] = colbase + col;
			col = (tile >> 24) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(6)] = colbase + col;
			col = (tile >> 20) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(5)] = colbase + col;
			col = (tile >> 16) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(4)] = colbase + col;
			col = (tile >> 12) & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(3)] = colbase + col;
			col = (tile >> 8)  & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(2)] = colbase + col;
			col = (tile >> 4)  & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(1)] = colbase + col;
			col = (tile >> 0)  & 0x0f; if (col != 0) bmap[PIXEL_XOR_BE(0)] = colbase + col;
		}
