...
public class system16
{
	...
	*/
	...
	static void draw_sprites( struct osd_bitmap *bitmap, int b3d ){
		...
		for( i=0; i<num_sprites; i++ ){
			...
			if( sprite.flags & SYS16_SPR_VISIBLE ){
				...
				if (flipx != 0) gfx += 2; else gfx += width/2;
				if (flipy != 0) gfx -= logical_height*width/2;
	...
	static void mark_sprite_colors( void ){
		...
		{
			UBytePtr pal = palette_used_colors;
			int i;
	...
	public static VhUpdatePtr sys16_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) {
		...
		if (sys16_bg_priority_mode != 0) tilemap_draw( bitmap, background, TILEMAP_IGNORE_TRANSPARENCY | 1, 0x00 );
	...
	//	sprite_draw(sprite_list,2);
		...
		if (sys18_fg2_active != 0) tilemap_draw( bitmap, foreground2, 0, 0x3 );
		tilemap_draw( bitmap, foreground, 0, 0x3 );
	//	sprite_draw(sprite_list,1);
		if (sys18_fg2_active != 0) tilemap_draw( bitmap, foreground2, 1, 0x7 );
	...
	public static VhUpdatePtr sys16_hangon_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) {
		if (!sys16_refreshenable) return;
		if (sys16_update_proc != 0) sys16_update_proc();
		update_page();
	...
	public static VhUpdatePtr sys16_outrun_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) {
	...
	public static VhUpdatePtr sys16_aburner_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) {
