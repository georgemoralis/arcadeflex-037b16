...
public class grchamp
{
	...
	static void draw_radar( struct osd_bitmap *bitmap ){
		...
		for( offs=0; offs<0x400; offs++ ){
			...
			if (data != 0){
				...
				int bit;
				for( bit=0; bit<8; bit++ ){
					if ((data & 0x80) != 0) plot_pixel( bitmap, x+bit, y, color );
					data <<= 1;
				}
			}
		}
	...
	*/
	...
	public static VhUpdatePtr grchamp_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) {
		...
		draw_rain( bitmap );
		draw_text( bitmap );
		if ((grchamp_videoreg0 & 0x80) != 0) draw_radar( bitmap );
