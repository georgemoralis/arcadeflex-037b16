...
public class unico16
{
	...
	***************************************************************************/
	...
	public static VhUpdatePtr unico16_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		int gunx[256] = {
	...
	if ( keyboard_pressed(KEYCODE_Z) || keyboard_pressed(KEYCODE_X) )
	{
		...
		if ((layers_ctrl & 4) != 0)	tilemap_draw(bitmap,tilemap_2,0,4);
	
		/* Sprites are drawn last, using pdrawgfx */
		if ((layers_ctrl & 8) != 0)	unico16_draw_sprites(bitmap);
	
		/* Draw the gunsight for ligth gun games */
		if (unico16_has_lightgun != 0)
