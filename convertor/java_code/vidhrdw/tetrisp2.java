...
public class tetrisp2
{
	...
	***************************************************************************/
	...
	{
		if (ACCESSING_LSB != 0)	data = (data >> 0) & 0xff;
	...
	***************************************************************************/
	...
	public static VhUpdatePtr tetrisp2_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		int i,layers_ctrl = -1;
	...
	//				tetrisp2_vregs[0x04/2],tetrisp2_vregs[0x06/2]
		...
		if ((layers_ctrl & 8) != 0)	for (i=0xf;i>=0;i--)	tetrisp2_draw_sprites(bitmap,i);
