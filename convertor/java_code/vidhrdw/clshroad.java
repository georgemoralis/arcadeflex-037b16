...
public class clshroad
{
	...
	public static VhConvertColorPromPtr clshroad_vh_convert_color_prom = new VhConvertColorPromPtr() { public void handler(char []palette, char []colortable, UBytePtr color_prom) 
	{
	} };
	...
	***************************************************************************/
	...
	public static GetTileInfoPtr get_tile_info_0a = new GetTileInfoPtr() { public void handler(int tile_index) 
	{
		data8_t code, color;
	...
	public static GetTileInfoPtr get_tile_info_0b = new GetTileInfoPtr() { public void handler(int tile_index) 
	{
		data8_t code, color;
		tile_index = (tile_index & 0x1f) + (tile_index & ~0x1f)*2;
	...
	***************************************************************************/
	...
	static void draw_sprites(struct osd_bitmap *bitmap)
	{
		...
		for (i = 0; i < spriteram_size ; i += 8)
		{
			...
			x -= 0x4a/2;
			if (flip_screen != 0)	{ y = max_y - y - 16;	flipx = !flipx;	flipy = !flipy;	}
	...
	***************************************************************************/
	...
	public static VhUpdatePtr clshroad_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		int layers_ctrl = -1;
	...
	//	memset(palette_used_colors,PALETTE_COLOR_USED,Machine.drv.total_colors);
		...
		if ((layers_ctrl & 1) != 0)	tilemap_draw(bitmap,tilemap_0a,0,0);	// Opaque
		else					fillbitmap(bitmap,Machine.pens[0],&Machine.visible_area);
	
		if ((layers_ctrl & 2) != 0)	tilemap_draw(bitmap,tilemap_0b,0,0);
	
		if ( (layers_ctrl & 8) && (priority & 1) )	draw_sprites(bitmap);
	
		if ((layers_ctrl & 4) != 0)	tilemap_draw(bitmap,tilemap_1,0,0);
