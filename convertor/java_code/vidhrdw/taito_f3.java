...
public class taito_f3
{
	...
	static void f3_clip_top_border(struct osd_bitmap *bitmap)
	{
		...
		for (i=0; i<256; i++) {
			...
			if (flipscreen != 0) pri_base-=2; else pri_base+=2;
		}
	}
	...
	/* These games either don't set the XY control bits properly (68020 bug?), or
		have some different mode from the others */
			...
			last_x=x;
			last_y=y;
			if (block_zoom_x != 0) x_addition=(0x110 - block_zoom_x) / 16; else x_addition=16;
			if (block_zoom_y != 0) y_addition=(0x110 - block_zoom_y) / 16; else y_addition=16;
			if (!sprite) continue;
			...
			if (block_zoom_x != 0) sprite_ptr.zoomx = ((0x10f-block_zoom_x)<<8)|0x80; else sprite_ptr.zoomx=0x10000;
			if (block_zoom_y != 0) sprite_ptr.zoomy = ((0x10f-block_zoom_y)<<8)|0x80; else sprite_ptr.zoomy=0x10000;
	...
	public static VhUpdatePtr f3_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		...
		if (flipscreen != 0) {
			...
			for (i=254; i>=0; i-=2) {
				...
				if ((ctrl & 0x1) != 0) rs[0]=(f3_line_ram[0x2800+(i>>1)]>>6);
				if ((ctrl & 0x2) != 0) rs[1]=(f3_line_ram[0x2880+(i>>1)]>>6);
				if ((ctrl & 0x4) != 0) rs[2]=(f3_line_ram[0x2900+(i>>1)]>>6);
				if ((ctrl & 0x8) != 0) rs[3]=(f3_line_ram[0x2980+(i>>1)]>>6);
				...
				if ((ctrl & 0x10000) != 0) rs[0]=f3_line_ram[0x2800+(i>>1)]>>22;
				if ((ctrl & 0x20000) != 0) rs[1]=f3_line_ram[0x2880+(i>>1)]>>22;
				if ((ctrl & 0x40000) != 0) rs[2]=f3_line_ram[0x2900+(i>>1)]>>22;
				if ((ctrl & 0x80000) != 0) rs[3]=f3_line_ram[0x2980+(i>>1)]>>22;
		...
		*/
		...
		for (pos=0; pos<4; pos++) {
			...
			need_t=need_z=1;
			if (flipscreen != 0) { t_pos=0xb1fe + (pos*0x200); z_pos=0x81fe + (pos*0x200); }
			...
			for (i=0; i<256; i++) {
				...
				if ((i&1 && !flipscreen) || ((i&1)==0 && flipscreen)) {
					if (need_t != 0) tpri[pos]=f3_line_ram[t_pos/4]&0xffff;
					if (need_z != 0) zoom[pos]=f3_line_ram[z_pos/4]&0xffff;
				}
				else {
					if (need_t != 0) tpri[pos]=f3_line_ram[t_pos/4]>>16;
					if (need_z != 0) zoom[pos]=f3_line_ram[z_pos/4]>>16;
				}
				...
				if (flipscreen != 0) { t_pos-=2; z_pos-=2; } else { t_pos+=2; z_pos+=2; }
		...
		*/
		sprite_pri_word=0;
		if (flipscreen != 0) { pos=0x5fe; pos2=0x77fe; inc=-2; } else { pos=0x400; pos2=0x7600; inc=2; }
		for (i=0; i<224; i++) { /* 224 instead of 256 as some games leave garbage at the bottom */
			...
			if (flipscreen != 0) { pos-=2; pos2-=2; } else { pos+=2; pos2+=2; }
	...
	public static VhEofCallbackPtr f3_eof_callback = new VhEofCallbackPtr() { public void handler() 
	{
