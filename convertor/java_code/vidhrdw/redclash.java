...
public class redclash
{
	...
	***************************************************************************/
	...
	{
		if ((data & 0xfe) != 0) usrintf_showmessage("5801 = %02x",data);
		gfxbank = data;
	} };
	...
	***************************************************************************/
	public static VhUpdatePtr redclash_vh_screenrefresh = new VhUpdatePtr() { public void handler(osd_bitmap bitmap,int full_refresh) 
	{
		int i,offs;
		...
		for (offs = spriteram_size[0] - 0x20;offs >= 0;offs -= 0x20)
		{
			...
			while (i > 0)
			{
				...
				if (spriteram.read(offs + i)& 0x80)
				{
					...
					if (spriteram.read(offs + i)& 0x10)
					{
						if (spriteram.read(offs + i)& 0x20)	/* 24x24 */
						{
							drawgfx(bitmap,Machine.gfx[3],
									((spriteram.read(offs + i + 1)& 0x70) >> 4) + ((gfxbank & 1) << 3) +
											((spriteram.read(offs + i + 1)& 0x80) >> 3),
									color,
									0,0,
									sx,sy - 16,
