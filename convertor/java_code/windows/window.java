...
public class window
{
	...
	extern extern UINT8 trying_to_quit;
	extern int verbose;
	...
	static static static double ddraw_compute_mode_score(int width, int height, int depth, int refresh);
	static static static static static static static static static void ddraw_compute_color_masks(const DDSURFACEDESC *desc);
	...
	static static void draw_debug_contents(HDC dc, struct osd_bitmap *bitmap);
	...
	int create_window(int width, int height, int depth, int attributes, int orientation)
	{
		...
		if (ddraw_stretch && use_ddraw)
		{
			...
			if ((attributes & VIDEO_ASPECT_RATIO_MASK) != 0)
			{
