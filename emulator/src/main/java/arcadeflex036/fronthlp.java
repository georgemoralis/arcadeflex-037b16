...
/*TEMPHACK*/ gamename = argv[1];//TO_BE_REMOVED (temp solution but works ok)
        ...
        for (i = 1; i < argc; i++) {
            ...
            if (stricmp(argv[i], "-gamelistheader") == 0) {
                list = LIST_GAMELISTHEADER; /* GAMELIST.TXT */
            }
            if (stricmp(argv[i], "-gamelistfooter") == 0) {
                list = LIST_GAMELISTFOOTER; /* GAMELIST.TXT */
            }
            if (stricmp(argv[i], "-gamelist") == 0) {
                list = LIST_GAMELIST; /* GAMELIST.TXT */
            }
            if (stricmp(argv[i], "-listgames") == 0) {
                list = LIST_LISTGAMES;
            }
            if (stricmp(argv[i], "-listclones") == 0) {
                list = LIST_LISTCLONES;
            }
            if (stricmp(argv[i], "-listsamdir") == 0) {
                list = LIST_LISTSAMDIR;
            }
            if (stricmp(argv[i], "-listcrc") == 0) {
                list = LIST_LISTCRC;
            }
            if (stricmp(argv[i], "-listdupcrc") == 0) {
                list = LIST_LISTDUPCRC;
            }
            if (stricmp(argv[i], "-listwrongmerge") == 0) {
                list = LIST_WRONGMERGE;
            }
            if (stricmp(argv[i], "-listromsize") == 0) {
                list = LIST_LISTROMSIZE;
            }
            if (stricmp(argv[i], "-listcpu") == 0) {
                list = LIST_LISTCPU;
            }
            if (stricmp(argv[i], "-lmr") == 0) {
                list = LIST_LMR;
            }
            if (stricmp(argv[i], "-wrongorientation") == 0) {
                list = LIST_WRONGORIENTATION;
            }
            if (stricmp(argv[i], "-wrongfps") == 0) {
                list = LIST_WRONGFPS;
            }
            if (stricmp(argv[i], "-gamexml") == 0) {
                list = CREATE_GAMEXML;
            }
            if (stricmp(argv[i], "-noclones") == 0) {
                listclones = 0;
            }
            ...
            if (strlen(gamename) > 0) {
                ...
                if (stricmp(argv[i], "-listsamples") == 0) {
                    list = LIST_LISTSAMPLES;
                }
                if (stricmp(argv[i], "-verifyroms") == 0) {
                    verify = VERIFY_ROMS;
                }
                if (stricmp(argv[i], "-verifysets") == 0) {
                    verify = VERIFY_ROMS | VERIFY_VERBOSE | VERIFY_TERSE;
                }
                if (stricmp(argv[i], "-vset") == 0) {
                    verify = VERIFY_ROMS | VERIFY_VERBOSE;
                }
                if (stricmp(argv[i], "-verifysamples") == 0) {
                    verify = VERIFY_SAMPLES | VERIFY_VERBOSE;
                }
                if (stricmp(argv[i], "-vsam") == 0) {
                    verify = VERIFY_SAMPLES | VERIFY_VERBOSE;
                }
                if (stricmp(argv[i], "-romident") == 0) {
                    ident = 1;
                }
                if (stricmp(argv[i], "-isknown") == 0) {
                    ident = 2;
                }
                if (stricmp(argv[i], "-sourcefile") == 0) {
                    list = LIST_SOURCEFILE;
                }
            }
        }

        if ((strlen(gamename) > 0) || list != 0 || verify != 0) {
            help = 0;
        }
            ...
            case LIST_LIST:
                ...
                while (drivers[i] != null) {
                    if ((listclones != 0 || drivers[i].clone_of == null
                            || ((drivers[i].clone_of.flags & NOT_A_DRIVER) != 0) /*TODO*/) /*&& !strwildcmp(gamename, drivers_old[i].name)*/) {
 ...
 /*TODO*/ //					if (strstr(name," (")) *strstr(name," (") = 0;
                ...
                return 0;
            case LIST_LISTSAMDIR:
                /* games list with samples directories */
...
/*TODO*/ // 					fprintf(stderr,"%d%%\r",100 * (i+1) / total);
 ...
 /* First, we shall print the header */
                ...
                for (j = 0; j < MAX_CPU; j++) {
                    printf("cpu_old %d    ", j + 1);
                }
                for (j = 0; j < MAX_SOUND; j++) {
                    printf("sound_old %d     ", j + 1);
                }
                printf("name\n");
                printf("-------- ---------- ");
                for (j = 0; j < MAX_CPU; j++) {
...
/*TODO*/ //				}
            ...
            case LIST_LISTGAMES:
                ...
                while (drivers[i] != null) {
                    if ((listclones != 0 || drivers[i].clone_of == null
                            || ((drivers[i].clone_of.flags & NOT_A_DRIVER) != 0) /*TODO*/) /*&& !strwildcmp(gamename, drivers_old[i].description)*/) {
                        char name[] = new char[200];
...
/*TODO*/ /*|| !strwildcmp(gamename,drivers_old[i].clone_of.name))*/) {
            ...
            case LIST_WRONGORIENTATION:
                /* list drivers_old which incorrectly use the orientation and visible area fields */
                while (drivers[i] != null) {
...
/*TODO*/ //				}
            ...
            case LIST_WRONGFPS:
                /* list drivers_old with too high frame rate */
...
/*TODO*/ //			}
            ...
            case LIST_LISTDUPCRC:
                /* list duplicate crc-32 (with different ROM name) */
                i = 0;
                while (drivers[i] != null) {
                    RomModule[] romp;
                    int romp_ptr = 0;
            ...
            case LIST_WRONGMERGE:
                /* list duplicate crc-32 with different ROM name in clone sets */
                i = 0;
                while (drivers[i] != null) {
                    RomModule[] romp;
                    int romp_ptr = 0;
