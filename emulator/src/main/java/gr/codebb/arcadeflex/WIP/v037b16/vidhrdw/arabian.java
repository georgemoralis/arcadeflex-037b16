...
public class arabian {
    ...
    public static VhConvertColorPromPtr arabian_vh_convert_color_prom = new VhConvertColorPromPtr() {
        public void handler(char[] palette, char[] colortable, UBytePtr color_prom) {
            ...
            for (i = 0; i < 64; i++) {
                palette[p_ptr++] = (char) (((i >> 5) & 1) * (153 * 192 / 255) + ((i >> 4) & 1) * (102 * 192 / 255) + ((i & 0x30) != 0 ? 63 : 0));
                palette[p_ptr++] = (char) (((i >> 3) & 1) * (156 * 192 / 255) + ((i >> 2) & 1) * (99 * 192 / 255) + ((i & 0x0c) != 0 ? 63 : 0));
                palette[p_ptr++] = (char) (((i >> 1) & 1) * 192 + ((i >> 0) & 1) * 63);
            }
    ...
    static void blit_area(int plane, int src, int x, int y, int sx, int sy) {
        ...
        for (i = 0; i <= sx; i++, x += 4) {
            for (j = 0; j <= sy; j++) {
                ...
                if ((plane & 0x01) != 0) {
                    if (p4 != 8) {
                        base.write(0, (base.read(0) & ~0xf0) | (p4 << 4));
                    }
                    if (p3 != 8) {
                        base.write(1, (base.read(1) & ~0xf0) | (p3 << 4));
                    }
                    if (p2 != 8) {
                        base.write(2, (base.read(2) & ~0xf0) | (p2 << 4));
                    }
                    if (p1 != 8) {
                        base.write(3, (base.read(3) & ~0xf0) | (p1 << 4));
                    }
                }
                ...
                if ((plane & 0x04) != 0) {
                    if (p4 != 8) {
                        base.write(0, (base.read(0) & ~0x0f) | p4);
                    }
                    if (p3 != 8) {
                        base.write(1, (base.read(1) & ~0x0f) | p3);
                    }
                    if (p2 != 8) {
                        base.write(2, (base.read(2) & ~0x0f) | p2);
                    }
                    if (p1 != 8) {
                        base.write(3, (base.read(3) & ~0x0f) | p1);
                    }
                }
            }
        }
