...
import static arcadeflex036.video.osd_get_pen;
import static common.libc.cstdlib.rand;
import common.ptr.UBytePtr;
import static gr.codebb.arcadeflex.WIP.v037b16.vidhrdw.vectorH.*;
...
public class vector {
    ...
    public static VhStartPtr vector_vh_start = new VhStartPtr() {
        public int handler() {
            ...
            flicker = (int) options.vector_flicker;//flicker;
            beam = options.beam;
...
/*TODO*///            osd_mark_vector_dirty(x, y);
    ...
    static void clever_mark_dirty() {
        ...
        for (i = min_index; i > 0; i--, oi++, ni++) {
            ...
            coords = new IntArray(pixel, (int) old_list[oi].arg1);
            int ci = 0;
            for (j = (old_list[oi].arg2 - old_list[oi].arg1); j > 0; j--) {
/*TODO*///                osd_mark_vector_dirty((int) coords.read(ci) >> 16, (int) coords.read(ci) & 0x0000ffff);
                ci++;
            }
        }
