...
public class _6522viaH {
    ...
    public static final int MAX_VIA = 8;
    ...
    public static abstract interface OutShiftPtr {

        public abstract void handler(int val);
    }

    public static abstract interface T2CallbackPtr {

        public abstract void handler(double tme);
    }

    public static abstract interface OutShift2Ptr {

        public abstract void handler(int val);
    }

    public static abstract interface SiReadyPtr {
    ...
    public static class via6522_interface {
        ...
        public via6522_interface(ReadHandlerPtr in_a_func, ReadHandlerPtr in_b_func, ReadHandlerPtr in_ca1_func, ReadHandlerPtr in_cb1_func, ReadHandlerPtr in_ca2_func, ReadHandlerPtr in_cb2_func, WriteHandlerPtr out_a_func, WriteHandlerPtr out_b_func, WriteHandlerPtr out_ca2_func, WriteHandlerPtr out_cb2_func, IrqPtr irq_func) {
        ...
        OutShiftPtr out_shift_func;
        T2CallbackPtr t2_callback;
        /* kludges for the Mac Plus (and 128k, 512k, 512ke) keyboard interface */
        OutShift2Ptr out_shift_func2;/* called when some data is shifted out in EXT sync mode */
        SiReadyPtr si_ready_func;/* called when the shift-in is enabled (EXT sync mode) */
    }
