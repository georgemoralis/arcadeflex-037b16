...
import static gr.codebb.arcadeflex.WIP.v037b16.drivers.system1.driver_wbml;
...
public class system1
{
	...
	static UBytePtr sprite_onscreen_map = new UBytePtr();
	...
	static UBytePtr wbml_paged_videoram = new UBytePtr();
	...
	***************************************************************************/
	...
	static int get_sprite_bottom_y(int spr_number)
	{
		return  spriteram.read(0x10 * spr_number + SPR_Y_BOTTOM);
	}
	
	static void draw_pixel(osd_bitmap bitmap,
	...
	{
		int[] bg_usage=new int[64], tx_usage=new int[64], sp_usage=new int[32];
		int i;
	
		memset (bg_usage, 0, bg_usage.length);
		memset (tx_usage, 0, tx_usage.length);
		memset (sp_usage, 0, sp_usage.length);
		...
		{
			if (bg_usage[i] != 0)
				memset (new UBytePtr(palette_used_colors, 1024 + i * 8), PALETTE_COLOR_USED, 8);
			else
				memset (new UBytePtr(palette_used_colors, 1024 + i * 8), PALETTE_COLOR_UNUSED, 8);
	
			palette_used_colors.write(512 + i * 8, PALETTE_COLOR_TRANSPARENT);
			if (tx_usage[i] != 0)
				memset (new UBytePtr(palette_used_colors, 512 + i * 8 + 1), PALETTE_COLOR_USED, 7);
			else
				memset (new UBytePtr(palette_used_colors, 512 + i * 8 + 1), PALETTE_COLOR_UNUSED, 7);
		}
		...
		for (i = 0; i < 32; i++)
		{
			...
			if (sp_usage[i] != 0)
				memset (new UBytePtr(palette_used_colors, 0 + i * 16 + 1), PALETTE_COLOR_USED, 15);
			else
				memset (new UBytePtr(palette_used_colors, 0 + i * 16 + 1), PALETTE_COLOR_UNUSED, 15);
		}
	...
	static void system1_draw_bg(osd_bitmap bitmap,int priority)
	{
		...
		if (priority == -1)
		{
			...
			{
				if (bg_dirtybuffer.read(offs / 2) != 0)
				{
					int code,color;
	
	
					bg_dirtybuffer.write(offs / 2, 0);
		...
		if (priority == -1)
		{
			...
			if (choplifter_scroll_x_on != 0)
			{
				if (flip_screen() != 0)
				{
					...
					copyscrollbitmap(bitmap,tmp_bitmap,32,scrollx_row_flip,0,new int[]{0},Machine.visible_area,TRANSPARENCY_NONE,0);
				}
				else
					copyscrollbitmap(bitmap,tmp_bitmap,32,scrollx_row,0,new int[]{0},Machine.visible_area,TRANSPARENCY_NONE,0);
			}
			else
...
/*TODO*///	#endif
	...
	{
		return wbml_paged_videoram.read(0x1000*wbml_videoram_bank + offset);
	} };
	
	public static WriteHandlerPtr wbml_paged_videoram_w = new WriteHandlerPtr() {public void handler(int offset, int data)
	{
		wbml_paged_videoram.write(0x1000*wbml_videoram_bank + offset, data);
	} };
