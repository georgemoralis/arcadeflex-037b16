...
public class cloud9 {
     ...
     * *************************************************************************
     */
    public static WriteHandlerPtr cloud9_paletteram_w = new WriteHandlerPtr() {
     ...
     *
     * Indirect Indexing: This is accomplished through memory locations 0x0000,
     * 0x0001, 0x00002. 0 = The x index on the bitmap 1 = The y index on the
     * bitmap 2 = The pixel value for that (x,y) location (this will be
     * 0x00-0x0F).
     *
     * Direct Indexing: Somehow, the bitmap is mapped to locations 0x600-0x3FFF.
     * Since a 256x256 bitmap would theoretically need 0x10000 bytes, we need a
     * way to compress it. We can reduce it to 0x8000 bytes by storing two
     * pixels to a byte. To drop it down to 0x4000 bytes, we're going to assume
     * there is two banks of video memory. Since Cloud9 doesn't provide many
     * examples for how these bytes are accessed, the following are guesses that
     * seem to work. If other games use this hardware, feel free to try
     * modifying the following assumptions. We'll assume that: X & 0x03 == 0 -
     * store at bank0, lo nibble X & 0x03 == 1 - store at bank0, hi nibble X &
     * 0x03 == 2 - store at bank1, lo nibble X & 0x03 == 3 - store at bank1, hi
     * nibble For Cloud9 to work, it's only important that writing 0's to
     * 0x0600-0x3FFF clears the bitmap. Oddly enough, to clear the bitmap, it
     * only writes to 0x0600-0x3FFF once, which leads me to believe there's a
     * register that allows both banks to be written to at once, and a register
     * to allow a specific bank to be selected.
     *
     **************************************************************************
     */
    /**
     ...
     * *************************************************************************
     */
    public static ReadHandlerPtr cloud9_bitmap_regs_r = new ReadHandlerPtr() {
     ...
     * *************************************************************************
     */
    public static WriteHandlerPtr cloud9_bitmap_regs_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            int x, y;
 ...
 /* Don't allow writes to memory at less than 0x600 */
     ...
     * *************************************************************************
     */
    public static WriteHandlerPtr cloud9_bitmap_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            int x, y;
    ...
    public static VhUpdatePtr cloud9_vh_screenrefresh = new VhUpdatePtr() {
