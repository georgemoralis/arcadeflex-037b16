...
import gr.codebb.arcadeflex.v037b16.generic.fucPtr;
...
/*TODO*///	extern void namcos1_set_sprite_offsets( int x, int y );
    ...
    static char[] key = new char[NAMCOS1_MAX_KEY];
	...
	*******************************************************************************/
	...
	public static class bankhandler {
		ReadHandlerPtr bank_handler_r;
		WriteHandlerPtr bank_handler_w;
		int 		  bank_offset;
		UBytePtr bank_pointer = new UBytePtr();
	};
	...
	static int /*mem_read_handler*/ org_bank_handler_r[] =
	{
	...
	static int /*mem_write_handler*/ org_bank_handler_w[] =
	{
	...
	*******************************************************************************/
	...
	//	logerror("CPU #%d PC %08x: keychip read %04X=%02x\n",cpu_getactivecpu(),cpu_get_pc(),offset,key[offset]);
        ...
        static short divider, divide_32 = 0;
        static short d;
	...
	public static WriteHandlerPtr rev1_key_w = new WriteHandlerPtr() {public void handler(int offset, int data) {
		...
		key[offset] = (char) data;
	
		switch ( offset )
		{
		case 0x01:
			divider = (short) (( key[0] << 8 ) + key[1]);
			break;
		case 0x03:
			{
	...
	*******************************************************************************/
				  ...
				  int offset,UBytePtr pointer)
	{
		...
		for(i=start;i<=end;i++)
		{
                        if (namcos1_bank_element[i] == null)
	...
	*******************************************************************************/
	...
	public static class namcos1_specific
	{
		...
		ReadHandlerPtr key_r;
		WriteHandlerPtr key_w;
		/* cpu slice timer */
		namcos1_slice_timer[] slice_timer;
