...
public class cloak {
     ...
     * *************************************************************************
     *
     * CLOAK & DAGGER uses RAM to dynamically create the palette. The resolution
     * is 9 bit (3 bits per gun). The palette contains 64 entries, but it is
     * accessed through a memory windows 128 bytes long: writing to the first 64
     * bytes sets the msb of the red component to 0, while writing to the last
     * 64 bytes sets it to 1.
     *
     * Colors 0-15 Character mapped graphics Colors 16-31 Bitmapped graphics
     * (maybe 8 colors per bitmap?) Colors 32-47 Sprites Colors 48-63 not used
     *
     * I don't know the exact values of the resistors between the RAM and the
     * RGB output, I assumed the usual ones. bit 8 -- inverter -- 220 ohm
     * resistor -- RED -- inverter -- 470 ohm resistor -- RED -- inverter -- 1
     * kohm resistor -- RED -- inverter -- 220 ohm resistor -- GREEN -- inverter
     * -- 470 ohm resistor -- GREEN -- inverter -- 1 kohm resistor -- GREEN --
     * inverter -- 220 ohm resistor -- BLUE -- inverter -- 470 ohm resistor --
     * BLUE bit 0 -- inverter -- 1 kohm resistor -- BLUE
     *
     **************************************************************************
     */
    public static WriteHandlerPtr cloak_paletteram_w = new WriteHandlerPtr() {
    ...
    public static WriteHandlerPtr cloak_clearbmp_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            bmap = data & 1;
            if ((data & 2) != 0) /* clear */ {
                if (bmap != 0) {
    ...
    public static ReadHandlerPtr graph_processor_r = new ReadHandlerPtr() {
    ...
    public static WriteHandlerPtr graph_processor_w = new WriteHandlerPtr() {
        public void handler(int offset, int data) {
            int col;
     ...
     **************************************************************************
     */
    public static VhStartPtr cloak_vh_start = new VhStartPtr() {
        public int handler() {
     ...
     **************************************************************************
     */
    public static VhStopPtr cloak_vh_stop = new VhStopPtr() {
        public void handler() {
            if (charbitmap != null) {
                bitmap_free(charbitmap);
            }
            if (tmpbitmap2 != null) {
    ...
    public static VhUpdatePtr cloak_vh_screenrefresh = new VhUpdatePtr() {
